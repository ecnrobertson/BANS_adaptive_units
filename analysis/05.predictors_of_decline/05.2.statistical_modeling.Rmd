...
modG, which has no AU-temperature interaction, only has a deviance explained of 30.8%. The temperature smooth is strongly significant but so is the AU random effect. So, temperature alone is explaining some variation but also AU differences in mean abundance trend exist. When we include AU specific smoothing parameters, as in all other models, the deviance explained goes up 4.3% so 35.1% overall (generally, across models). So yes, it's a better model when you allow for AU specific responses.
