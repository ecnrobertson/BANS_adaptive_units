...
vcftools --vcf $OUT/BANS.all.ds6x_gatk.SNP.vcf --out $OUT/BANS.all.ds6x_gatk.SNP.05maf --min-alleles 2 --max-alleles 2 --max-missing 0.5 --maf 0.05 --recode --recode-INFO-all
...
-R $REFERENCE \
-V $OUT/BANS.all.ds6x_gatk.SNP.05maf.recode.vcf \
...
java -Djava.io.tmpdir=/scratch/alpine/ericacnr@colostate.edu/tmp -Xmx37g -jar /curc/sw/install/bio/gatk/4.3.0.0/gatk-package-4.3.0.0-local.jar  SelectVariants \
     ...
     -O $OUT/BANS.all.ds6x_gatk.SNP.05maf.VQSR2.PASS.vc
...
vcftools --vcf $OUT/BANS.all.ds6x_gatk.SNP.05maf.VQSR2.PASS.vcf --out $OUT/BANS.all.ds6x.pass-maf-0.05.bialSNP.filtered.0.8miss.recode.vcf --max-missing 0.8 --recode --recode-INFO-all
```

Yay! Head on over to vcf_eval to do some tests for batch effects and missingness stuff.
...
```{bash}
vcftools --vcf BANS.all.ds6x_gatk.SNP.05maf.VQSR2.PASS.vcf --missing-indv
...
<!-- After filtering, kept 9722479 out of a possible 9722479 Sites -->

#filt ind with higher than 50% overall missingness
awk 'NR>1 && $5 > 0.5 {print $1}' out.imiss > bad_ind
wc -l bad_ind

# then filter SNP level missingness
vcftools --vcf BANS.all.ds6x_gatk.SNP.05maf.VQSR2.PASS.vcf --remove bad_ind --out BANS.all.ds6x.pass-maf-0.05.bialSNP.filtered.ind0.3miss.0.8miss --max-missing 0.8 --recode --recode-INFO-all

<!-- After filtering, kept 215 out of 229 Individuals -->
<!-- Outputting VCF file... -->
<!-- After filtering, kept 4069269 out of a possible 9722479 Sites -->

vcftools --vcf BANS.all.ds6x.pass-maf-0.05.bialSNP.filtered.ind0.3miss.0.8miss.recode.vcf --missing-indv

bcftools stats BANS.all.ds6x.pass-maf-0.05.bialSNP.filtered.ind0.3miss.0.8miss.recode.vcf > BANS.all.ds6x.pass-maf-0.05.bialSNP.filtered.ind0.3miss.0.8miss.recode.stats

awk 'NR>1 {sum += (1-$5)} END {print sum/NR}' out.imiss
#0.850813
```

That is, in fact, what happened! 4069269 with this type of filtering. 215 samples left.
