...
You will want to take the filtered, imputed, ld-pruned vcf you used for pop str and further filter individuals. If you do not have snp ids, create them like so. Then calculate per population allele frequency with plink.
```{bash}

plink --vcf 311ind-pass-maf05-snp-miss50-scafmt10-gtglimpute4.1-exclLD50-5-0.5.vcf.gz --keep sample_list_269.tx --recode --aec --out 311to269ind-pass-maf05-snp-miss50-scafmt10-gtglimpute4.1-exclLD50-5-0.5

head 311to269-pass-maf05-snp-miss50-scafmt10-gtglimpute4.1-exclLD50-5-0.5.map
...
awk '{if ($2 == ".") $2 = $1 ":" $4; print }' 311to269-pass-maf05-snp-miss50-scafmt10-gtglimpute4.1-exclLD50-5-0.5.map > tmp && mv tmp 311to269-pass-maf05-snp-miss50-scafmt10-gtglimpute4.1-exclLD50-5-0.5.map

head 311to269ind-pass-maf05-snp-miss80-scafmt10-gtglimpute4.1-exclLD50-5-0.5.map
...
plink --file 311to269-pass-maf05-snp-miss80-scafmt10-gtglimpute4.1-exclLD50-5-0.5 --make-bed --aec --out 311to269-pass-maf05-snp-miss80-scafmt10-gtglimpute4.1-exclLD50-5-0.5

# now i should be able to create the pop allele frequency file
plink --bfile 311to269-pass-maf05-snp-miss80-scafmt10-gtglimpute4.1-exclLD50-5-0.5 --freq --within sample_list_34pop.txt --aec --out 311to269-pass-maf05-snp-miss80-scafmt10-gtglimpute4.1-exclLD50-5-0.5
```
...
df <- fread("311to269-pass-maf05-snp-miss80-scafmt10-gtglimpute4.1-exclLD50-5-0.5.frq.stat", header=TRUE)
set.seed(42)
# set.seed(43)
# set.seed(44)
...
write.table(as.data.frame(subset50k_wide), file="losh.34pop.frq.strat.50k.txt", row.names=F, quote=F, sep="\t")
...
Run on alpine!
```{r}
pops <- read.table(file = "sample_list_34pop.tsv", sep="\t", header = FALSE) %>% rename(BGP_ID = V1, Pop = V3) %>% dplyr::select(BGP_ID, Pop)

clim <- read_delim("env-variables/LOSH.269ind.34pop.worldclim.txt", delim = "\t") %>% 
  left_join(pops, by = "BGP_ID") %>%
  dplyr::select(starts_with("wc2.1_30s_bio_"), Pop) %>%
  group_by(Pop) %>% 
  summarise(n=n(),across(starts_with("wc2.1_30s_bio_"), mean)) %>% 
  filter(n>2)

hii <- read_delim("env-variables/LOSH.269ind.34pop.hii.txt", delim = "\t") %>% 
  left_join(pops, by = "BGP_ID") %>%
  dplyr::select(NA_Human_Influence_Index, Pop) %>% 
  filter(!is.na(NA_Human_Influence_Index)) %>% # just drop the one pesky bird
  group_by(Pop) %>% 
  summarise(n=n(), mean(NA_Human_Influence_Index)) %>% 
  filter(n>2)

ndvi.max <- read_delim("env-variables/LOSH.269ind.34pop.ndvi.max.txt", delim = "\t") %>% 
  left_join(pops, by = "BGP_ID") %>%
  dplyr::select(ndvimax, Pop) %>% 
  group_by(Pop) %>% 
  summarise(n=n(), mean(ndvimax)) %>% 
  filter(n>2)

ndvi.sd <- read_delim("env-variables/LOSH.269ind.34pop.ndvi.sd.txt", delim = "\t") %>% 
 left_join(pops, by = "BGP_ID") %>%
  dplyr::select(ndvistd, Pop) %>% 
  group_by(Pop) %>% 
  summarise(n=n(), mean(ndvistd)) %>% 
  filter(n>2)

landcover_buffer <- read_delim("env-variables/LOSH.269ind.34pop.perclandcover.1kmradius.txt", delim = "\t")
```
...
```{bash}
awk 'NR>1 { if(!seen[$3]++) print $3 }' 311to269-ind-pass-maf05-snp-miss50-scafmt10-gtglimpute4.1-exclLD50-5-0.5.frq.strat > 311to269ind-frq-pop-order.txt
```

```{r}
## reorder so that env and alle frequencies per pop are reported in the same order
poporder <- read.table(file = "311to269ind-frq-pop-order.txt", header = F) %>% rename(Pop = V1)
clim_ordered <- poporder %>% left_join(clim) %>% dplyr::select(-n)
colnames(clim_ordered) <- c(
  "Pop",
  paste0("Bio", seq_len(ncol(clim_ordered) - 1))
)
...
Elosh <- left_join(clim_ordered, hii_ordered, by = "Pop")
Elosh <- left_join(Elosh, landcover_ordered, by = "Pop")
Elosh <- left_join(Elosh, ndvi.max_ordered, by = "Pop")
Elosh <- left_join(Elosh, ndvi.sd_ordered, by = "Pop") %>% dplyr::select(-Pop)
Elosh
#Elosh <- clim_ordered %>% dplyr::select(-Pop)
#write.table(Elosh, file = "311to269ind.clim.hii.lc500m.ndvi.txt", row.names=F, quote=F, sep="\t")
```
...
library(data.table)
library(gradientForest)
Elosh <- read.table(file="311to269ind.clim.hii.lc500m.ndvi.txt", sep="\t", header=T)
Glosh <- fread("losh.34pop.frq.strat.50k.txt",sep="\t")
dim(Glosh)
...
loshforest=gradientForest(cbind(Elosh,Glosh), 
...
saveRDS(loshforest,file="losh.clim.hii.lc500km.ndvi.GF_results.50kb_A.rds")
...
ranked_predictors <- c("Bio18", "Bio12", "Bio7", "Bio16", "Bio13", "Bio8", "mean(ndvimax)", "Bio14", "Bio17", "Shrubland", "Bio19", "Cropland", "Bio2", "Bio4")
